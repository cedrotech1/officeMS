...
if (isset($_GET['ajax_user_list'])) {
  ...
  $where = '1=1';
  $where .= " AND campus = '".mysqli_real_escape_string($connection, $mycampus)."'";
  $where .= " AND role != 'officer'";
  if ($search !== '') {
  ...
  // Clean output buffer to prevent whitespace or warnings before JSON
    ...
    if (($handle = fopen($file, 'r')) !== false) {
      ...
      if (count($missing) > 0) {
        ...
        while (($row = fgetcsv($handle)) !== false) {
          ...
          // Optional image
          $rowAssoc['image'] = (isset($headerMap['image']) && isset($row[$headerMap['image']]) && trim($row[$headerMap['image']]) !== '') ? trim($row[$headerMap['image']]) : 'upload/icon1.png';
          // Validation
        ...
        foreach ($data as $rowData) {
          ...
          $image = isset($rowData['image']) && $rowData['image'] !== '' ? mysqli_real_escape_string($connection, $rowData['image']) : 'upload/icon1.png';
          $campus = strtolower(trim($rowData['campus'] ?? ''));
          if ($campus === '' || $campus === null) {
            $campus = $mycampus;
          }
          // Check for existing email or phone
          $check = mysqli_query($connection, "SELECT id FROM users WHERE email = '$email' OR phone = '$phone' LIMIT 1");
          if (mysqli_num_rows($check) > 0) {
            $errors[] = "Email or phone already exists for user $names ($email, $phone).";
          }
          if (!in_array($campus, $allowed_campuses)) {
            $errors[] = "Invalid campus '$campus' for user $names. Must be one of: ".implode(', ', $allowed_campuses);
          }
          if ($campus !== $mycampus) {
            $errors[] = "Campus for user $names must match your campus: $mycampus.";
          }
        }
        if (count($errors) > 0) {
          ...
          foreach ($data as $rowData) {
            ...
            $role = mysqli_real_escape_string($connection, $rowData['role']);
            $image = isset($rowData['image']) && $rowData['image'] !== '' ? mysqli_real_escape_string($connection, $rowData['image']) : 'upload/icon1.png';
            $password = password_hash('23122312', PASSWORD_BCRYPT);
            $campus = strtolower(trim($rowData['campus'] ?? ''));
            if ($campus === '' || $campus === null) {
              $campus = $mycampus;
            }
            mysqli_query($connection, "INSERT INTO users (names, email, phone, role, image, campus, password, active) VALUES ('$names', '$email', '$phone', '$role', '$image', '$campus', '$password', 1)");
          }
...
if (isset($_GET['ajax_office_list'])) {
    ...
    $where = '1=1';
    $where .= " AND campus_name = '".mysqli_real_escape_string($connection, $mycampus)."'";
    if ($search !== '') {
...
if (isset($_GET['ajax_room_users']) && isset($_GET['room'])) {
  ...
  $users = mysqli_query($connection, "SELECT users.*, allocation.id AS alloc_id, allocation.status as alloc_status, offices.* FROM allocation JOIN users ON allocation.user_id = users.id JOIN offices ON allocation.office_id = offices.id WHERE offices.room_code LIKE '%$room%' AND offices.campus = '".mysqli_real_escape_string($connection, $mycampus)."' ORDER BY users.names");
  $user_rows = [];
  if ($users) {
    while ($u = mysqli_fetch_assoc($users)) {
...
<head>
  ...
  <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <style>
        ...
        .loading:after { content: 'Loading...'; position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); background: #fff; padding: 10px; border-radius: 5px; z-index: 10; }
        .spinner-overlay {
          position: absolute;
...
<main id="main" class="main">
  ...
  <section class="section dashboard">
    <div class="row card p-3">
      ...
      <div class="mb-3">
        <button class="btn btn-success" type="button" id="toggleUploadUsersForm"><i class="bi bi-upload"></i> Upload Users (CSV/Excel)</button>
      </div>
      <!-- Make the upload users card form col-md-4 and centered -->
      <div class="row">
        <div class="col-md-6">
          <div class="card mb-3 p-3 shadow-sm" id="uploadUsersCard" style="display:none;background-color:whitesmoke">
      ...
      <div class="card mb-3 p-3 shadow-sm">
        ...
        <div id="userTableLoading" class="spinner-overlay" style="display:none;">
...
<script>
function showUserTableLoading() {
  $("#userTableLoading").show();
}
function hideUserTableLoading() {
  $("#userTableLoading").hide();
}
$(document).ready(function() {
  ...
  // Robust remove allocation handler
  function removeAllocation(allocId, officeId = '') {
    showUserTableLoading();
    $.ajax({
        url: 'allocate_office.php',
        method: 'POST',
        data: {
            ajax_action: 'remove',
            alloc_id: allocId,
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            hideUserTableLoading();
            let msg = 'AJAX error: ' + textStatus;
            if (jqXHR.responseText) {
                try {
                    let err = JSON.parse(jqXHR.responseText);
                    msg += ' - ' + (err.error || err.message || jqXHR.responseText);
                } catch (e) {
                    msg += ' - ' + jqXHR.responseText;
                }
            }
            alert(msg);
            console.error('AJAX error:', jqXHR, errorThrown);
        }
    });
  }

  var removeAllocId = null;
  var removeOfficeId = null;
  ...
  $(document).on('click', '.remove-office-btn', function() {
    removeAllocId = $(this).data('alloc-id');
    removeOfficeId = $(this).data('office-id') || '';
    removeModal.show();
  });
  $('#confirmRemoveBtn').on('click', function() {
    if (removeAllocId) {
      removeAllocation(removeAllocId, removeOfficeId);
      removeModal.hide();
    }
  });
  ...
  // Toggle upload users form
  $('#toggleUploadUsersForm').on('click', function() {
    $('#uploadUsersCard').slideToggle();
  });
