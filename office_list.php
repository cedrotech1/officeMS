      ...
      <div class="col-md-2">
        <select name="campus" class="form-control">
          ...
          <?php foreach ($campuses as $c): ?>
            <option value="<?= htmlspecialchars($c) ?>" <?= $campus===$c?'selected':'' ?>><?= htmlspecialchars($c) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-md-2">
...
function loadOffices(page=1, view=null) {
  ...
  const campus = $('select[name=campus]').val();
...
$(document).ready(function() {
  ...
  $('select[name=campus], select[name=occupancy], input[name=occupants_num], input[name=search]').on('change input', function() {
    loadOffices(1);
  });
