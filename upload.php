...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['jsonData'])) {
  ...
  foreach ($data as $row) {
    ...
    $q = mysqli_query($connection, "SELECT COUNT(*) as cnt FROM offices WHERE room_code='$roomCodeEsc'");
    $r = mysqli_fetch_assoc($q);
    if ($r['cnt'] > 0) {
      $errors[] = "Room code '$roomCode' at row $rowNum already exists in the database.";
    }
